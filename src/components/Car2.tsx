...
function Car2({ position = [0, 0, 0], path, speed = 2 }: Car2Props) {
  const carRef = useRef<Group>(null)
  const { scene } = useGLTF('/hunyuan3d_4152d885.glb')
  ...
  return (
    <group ref={carRef} position={position}>
      <primitive object={clonedScene} scale={[0.5, 0.5, 0.5]} rotation={[0, Math.PI / 2 + Math.PI, 0]} />
    </group>
  )
}
